 ...
 * a 128x64 pixel SSD1306-based OLED display that comes on the Heltec V3 board. */
#define OLED_WIDTH          128
#define OLED_HEIGHT         64

/* The SSD1306 display is organized in pages, each page is 8 pixels high.
 * The number of pages for a 64 pixel high display is 8 (64 / 8 = 8). */
#define SSD1306_PAGE_HEIGHT 8

/* Number of pages for a 128x64 SSD1306 display (8 pages) */
#define SSD1306_128x64_NUM_OF_PAGES (OLED_HEIGHT / SSD1306_PAGE_HEIGHT)

/* Number of lines for a 128x64 SSD1306 display based on the font height */
...
#define SSD1306_128x64_NUM_OF_LINES (OLED_HEIGHT / FONT_HEIGHT)
 ...
 * For example, for a 128x64 display, the first page (page 0) starts at index 0,
...
/* OLED RST pin configuration */
#define OLED_RST_PIN GPIO_NUM_21 // As per Heltec V3.1 schematic, RST pin is GPIO21
...
esp_err_t oled_init(void)
{
    ...
    esp_lcd_panel_dev_config_t panel_config =
    {
        ...
        .reset_gpio_num = OLED_RST_PIN, // As per Heltec V3.1 schematic, RST pin is GPIO21
...
esp_err_t oled_refresh(void) 
{
     ...
     * 
     ******************* Let's understand how to oled_buffer is mapped to our SSD1306 128x64 display:***************************
     *
     * The `oled_buffer` is a linear array of `uint8_t` with a size of `OLED_WIDTH * (OLED_HEIGHT / 8)`,
     * which is 128 * (64 / 8) = 1024 bytes for this display.
     * The SSD1306 organizes its display memory (GDDRAM) into "pages". Each page consists of 8 rows of pixels.
     * For a 128x64 display, there are 64 rows / 8 rows/page = 8 pages (Page 0 to Page 7).
     *
     * Buffer Layout:
...
{
    /* Make sure the line number is valid (0 to SSD1306_128x64_NUM_OF_LINES - 1) for the current font and display size */
    if (line >= SSD1306_128x64_NUM_OF_LINES) 
    {
...
static void oled_draw_char(int x, int y, char char_to_draw) 
{
    ...
    for (int font_col = 0; font_col < FONT_WIDTH; font_col++) 
    {
        ...
        for (int font_row = 0; font_row < FONT_HEIGHT; font_row++) 
        {
             ...
             **/
            int page = screen_y / SSD1306_PAGE_HEIGHT; // Page number (0-3 for 128x64 display)
            ...
            uint16_t buf_idx = screen_x + (page * OLED_WIDTH); // Buffer index (0-511 for 128x64 display)
